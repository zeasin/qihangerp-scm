    ...
    <el-row>
      <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="108px">
        ...
        <el-form-item label="面单类型" prop="type">
          <el-select v-model="queryParams.type" filterable  placeholder="面单类型">
            <el-option label="自有" value="0"></el-option>
            <el-option label="商户共享" value="1"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="商户" prop="merchantId">
          <el-select v-model="queryParams.merchantId" placeholder="请选择商户" clearable @change="handleQuery">
            <el-option value="0" label="总部"></el-option>
            <el-option
...
<!--      <el-table-column label="ID" align="center" prop="id" />-->
      <el-table-column label="面单类型" align="center" prop="type" >
        <template slot-scope="scope">
          <el-tag size="small" v-if="scope.row.type===0">自有</el-tag>
          <el-tag size="small" v-if="scope.row.type===1">商户共享</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog title="发货人&快递模版设置" :visible.sync="updateOpen" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="网点编码" prop="branchCode">
          <el-input v-model="form.branchCode" placeholder="请输入网编码" />
        </el-form-item>
        <el-form-item label="快递模块" prop="templateUrl">
          ...
          <el-button
            size="mini"
            type="text"
            @click="handlePullTemplate(form.wpCode)"
          >更新快递模版</el-button>
        </el-form-item>
...
import {listMerchant,listMerchantShop} from "@/api/vms/merchant";
import {
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row){
      ...
      this.form.branchCode = row.branchCode
      // this.form.sellerShopId = row.sellerShopId
      this.form.name = row.name
      this.form.mobile = row.mobile
      this.form.wpCode = row.wpCode
      getWaybillTemplateList({wpCode:row.wpCode}).then(resp=>{
